   ...
   return (
      <div className='stream box border border-gray-800 overflow-hidden'>
         ...
         <div className='stream-info p-6'>
            ...
            { data.platformtype === 'youtube' ? <a target='_blank' rel="noreferrer" href={`https://www.youtube.com/watch?v=${data.streamId}`} className={` btn btn-main sm flex items-center justify-center`}><svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            {data.endedAt ? "" :
               <>
               <button onClick={()=>endStream(data.streamId)} className={`${isLive ? "" :  "sdisabled"} mt-3 btn w-full btn-main sm flex items-center justify-center`}>
