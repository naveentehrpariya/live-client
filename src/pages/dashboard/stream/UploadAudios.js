...
export default function UploadAudios({update, streamType}) {
  ...
  return (
    <div className='pt-4'>
      ...
      <div className='flex mb-2'>
        <button onClick={()=>setAudioType("music")} className={`${audioType === 'music' ? 'bg-main text-white' : 'bg-gray-600 text-gray-300'}  uppercase rounded-xl px-3 py-1 me-3 mb-3 text-[13px]`}>Songs Lists</button>
        <button onClick={()=>setAudioType("radio")} className={`${audioType === 'radio' ? 'bg-main text-white' : 'bg-gray-600 text-gray-300'}  uppercase rounded-xl px-3 py-1 me-3 mb-3 text-[13px]`}>Radio Stream</button>
      </div>
      ...
      : 
        <input required name='radiostream' type={'text'} placeholder={"Radio stream url"} className="input" />
      }
